...
for (const page of ['math-rush.html', 'math-map.html']) {
  ...
  assert.match(src, /data-board-home/, `${page} map has a one-tap home button`);
...
assert.match(boardJs, /data-board-home/, 'Home button returns the board to its initial view');
...
assert.match(runnerPage, /page-runner/, 'Runner page locks to the viewport');
assert.match(runnerPage, /RingGeometry/, 'Smashes get shockwave rings');
assert.doesNotMatch(runnerPage, /Как играть/, 'Runner page has no scrolling manual');
